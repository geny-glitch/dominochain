...
class ChessComConfig < ApplicationRecord
  belongs_to :user
  ...
  def stored_scenario_set
    @stored_scenario_set ||= ScenarioSet.from_hash(self[:scenarios], source: :chess)
  end

  def chess_allowed
    BetaEvents::ScenarioRegistry.allowed_actions_for(:chess)
  end
  ...
  def scenarios_are_valid
    scenario_set.scenarios.each do |scenario|
      if scenario.event == "goal_failed"
        ...
        if goal_id <= 0 || !user.chess_com_goals.exists?(id: goal_id)
          errors.add(:scenarios, :invalid)
          next
        end
      end

      sanction = scenario.to_sanction_set(allowed: chess_allowed)
