...
class StravaGoalCheck < ApplicationRecord
  ...
  validates :week_start_on, :week_end_on, :checked_at, presence: true
  ...
  validates :week_start_on, uniqueness: { scope: :strava_goal_id }

  scope :recent, -> { order(week_start_on: :desc, checked_at: :desc) }
end
