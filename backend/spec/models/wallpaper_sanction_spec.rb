...
RSpec.describe WallpaperSanction do
  it "parses unified lock fields" do
    ...
    expect(sanction.to_h["leverage_photo_lock_photo_id"]).to eq(42)
    expect(sanction.to_h["leverage_photo_delete_photo_id"]).to be_nil
  end
  ...
  it "coalesces legacy start/add_time into lock (start preferred)" do
    ...
    expect(sanction.leverage_photo_lock_target_mode).to eq("specific")
    expect(sanction.leverage_photo_lock_photo_id).to eq(42)
    expect(sanction.to_h).not_to have_key("leverage_photo_start_enabled")
  end
